...
flèches / WASD (clavier) ; sur tactile, **4 schémas au choix** (Options, persisté
`ct_controls`, défaut **Glisser**) — voir « Contrôles tactiles » ci-dessous.
...
│   ├── input.js        # CT.Input — clavier / tactile (4 schémas : dpad/swipe/zones/joystick) → callbacks
...
`setDir` accepte aussi **`'cw'` / `'ccw'`** (rotation **relative** à la référence de file,
utilisée par les Zones tactiles) ; le **journal de rejeu** encode toujours la direction
**absolue résultante** (cohérent quel que soit le mode d'entrée).

### Contrôles tactiles (`js/input.js` → `CT.Input`, 4 schémas)
Sur tactile, le joueur choisit son schéma dans les **Options** (« 🎮 Contrôles tactiles »,
persisté `localStorage ct_controls`, défaut **`swipe`**). `CT.Input.setScheme(s)` / `getScheme()` ;
les écouteurs tactiles sont posés **une fois** sur le canvas et **branchent selon le schéma courant**
(pas de rebind). Schémas :
- **`dpad`** : D-pad tactile à l'écran (`#dpad`, boutons ▲◀▶▼ → `onDir` absolu). Seul schéma qui
  **affiche** le D-pad ; les autres le cachent (`applyScheme`).
- **`swipe`** (défaut) : glisser le doigt sur le plateau. **Continu** — l'origine est **ré-ancrée**
  à chaque virage déclenché (seuil `THRESH`) → on garde le doigt posé et on enchaîne les directions
  dans un seul geste. Un flick rapide (sans `touchmove`) déclenche aussi un virage à `touchend`.
- **`zones`** : **taper** la moitié **gauche** → `onDir('ccw')` (tourner à gauche), moitié **droite**
  → `onDir('cw')` (tourner à droite) — rotation **relative**, jouable au pouce. Un glissé net vaut un
  virage direct.
- **`joystick`** : un **joystick flottant** (`#joystick`, base + knob, positionné en JS) apparaît sous
  le doigt à `touchstart`, le knob suit (clampé au rayon `JOY_R`), la direction dominante pilote ;
  masqué à `touchend`.
...
- **19 améliorations** (`CT.Lab.UPGRADES`, plusieurs niveaux) : Surtension (+10 %
  ...
  **par niveau terminé** via `mods.levelBonus`, cumulées dans `game.levelBonusCoins` à chaque
  `startCinematic` puis versées au Labo à la mort → banque, pas au score), **Prime de survie** ⌛
  (+5 ⚡/niv **par 30 s survécues** via `mods.survivalBonus`, calculées à la mort sur la durée
  `game.survivalCoins` → récompense aussi le Chrono ; banque, pas au score), **Labo accéléré** ⏩ (−5 %/niv de
